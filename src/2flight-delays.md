...
<h4 style="color:grey"><i>To what extent did the COVID-19 pandemic impact air traffic operations, specifically in terms of delays?</i></h4>
...
function filterData(year, month) {
  return datasetFlights.filter(d => {
    ...
    return flightDate.getFullYear() === +year &&
      (month === "All" || flightDate.getMonth() === availableMonths.indexOf(month) - 1);
  });
}
...
function drawHeatmap() {
  ...
  const avgDelays = heatmapData.map(d => d.avgDelay).sort(d3.ascending);
  //const percentiles = d3.range(1, 7).map(p => d3.quantile(avgDelays, p / 7));
  //const colors = d3.schemeRdYlGn[7].slice().reverse(); // Red to Green scale (reversed to match delays)
  //colors[3] = "#ffffff"; // Sostituisci il colore centrale con bianco
  //const colorScale = d3.scaleThreshold().domain(percentiles).range(colors);
  ...
  const delayColors = d3.schemeRdYlGn[7].slice().reverse(); // Red to Green scale (reversed to match delays)
  delayColors[3] = "#ffffff"; // Sostituisci il colore centrale con bianco
  //const delayColors = ["green", "lightgreen", "white", "lightcoral", "red"];

  const colorScale = d3.scaleThreshold()
  .domain(thresholds.slice(1, -1)) // exclude the first and last (they’re implied)
  .range(delayColors);
...
for (let i = 0; i < thresholds.length - 1; i++) {
  ...
  const isZeroRange = start === 0 && end === 0;
  ...
  legendGroup.selectAll("text")
    ...
    .attr("x", (d, i) => i * 60)
    .attr("y", 35)
    //.text((d, i) => `${legendData[i].toFixed(1)}${i < legendData.length - 1 ? " – " + legendData[i + 1].toFixed(1) : "+"}`)
    .text(d => d.label)
    .style("fill", "white")
  ...
  legendGroup.append("text")
    .attr("x", 135)
    .attr("y", -10) // position above the color boxes
    .text("Average Arrival Delay (mins)")
    .style("fill", "white")
...
function toggleAutoplay() {
  ...
  if (isAutoplayActive) {
    ...
    autoplayInterval = setInterval(() => {
      ...
      let nextYear = availableYears[(currentYearIndex + 1) % availableYears.length];
...
Notably, the heatmap does not show clear temporal patterns that would suggest a strong impact of the COVID-19 pandemic on delays. When observing the weekly and hourly segments throughout the 2019–2023 period, there are no significant disruptions that align with the global shutdown phase. This suggests that while flight volumes may have decreased during the pandemic, the average delay patterns remained relatively stable, without notable spikes or anomalies directly attributable to COVID-19 restrictions.

</div>
<br>

## Number of Delays by Reason
<h4 style="color:grey"><i>Excluding the impact of the COVID-19 pandemic, what have been the main factors responsible for delays in air transport in recent years?</i></h4>
...
The bar chart indicates that carrier-related issues were the leading cause of flight delays, significantly surpassing other categories. Among all causes, delays attributed to carriers show the highest frequency.
<br>
Furthermore, a notable concentration of these delays occurred during the summer season. This seasonal spike is likely due to increased passenger volume during the summer months, driven by vacation travel and heightened air traffic demand.
<br>
However, this visualization enables users to discern which factors predominantly contribute to delays and how their impact varies seasonally, facilitating targeted strategies to mitigate specific delay causes.
</div>
...
<h4 style="color:grey"><i>Is there a significant correlation between the flight distance and the incidence of delays across different airlines?</i></h4>
...
This chart assists in understanding whether longer flights are more susceptible to delays and how each airline's performance compares in this context, offering insights into operational efficiencies related to flight length.
<br>
However, as shown in the graph, longer flight routes do not necessarily experience greater delays; in fact, they often have significant early arrivals. This is likely due to the fact that the scheduled arrival times for longer routes are set with a wider margin, allowing for more flexibility and, in some cases, earlier arrivals than expected.</div>
<br>
<br>


## Flight Delays per company
<h4 style="color:grey"><i>To what extent does the geographical location of airports influence the frequency and severity of flight delays?</i></h4> 
...
const colorScale = d3.scaleDiverging()
  .domain([-20, 0, 210]) // Negative = Early, 0 = On Time, 30+ = Very Late
  //.interpolator(d3.interpolateRgbBasis(["green", "white", "red"]));
  .interpolator(d3.interpolateRgbBasis(["green", "lightgreen", "white", "lightcoral", "red"]));
...
svg.append("g")
  ...
  .on("mouseover", function(event, d) {
    d3.select("#airport-map-container").selectAll("circle").style("opacity", 0.3); 
    d3.select(this)
      .style("opacity", 1) 
      .attr("stroke", "white");
...
function updateMap() {
  ...
  legend.append("defs")
    ...
    .selectAll("stop")
    .data([
      { offset: "0%", color: colorScale(-20) }, // Green (Early)
